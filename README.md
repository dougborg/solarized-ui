...
| `.panel-rows` | Put on a panel `ul`/`ol`, or on a panel whose direct child is a `table` (with a wrapper between them, the outer rows lose their curve and the body groups their dividers): rows with dividers, title on the start side and a value or status on the end side |
